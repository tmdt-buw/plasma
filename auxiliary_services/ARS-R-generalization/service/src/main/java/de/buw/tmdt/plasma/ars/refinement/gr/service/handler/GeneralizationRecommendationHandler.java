package de.buw.tmdt.plasma.ars.refinement.gr.service.handler;

import de.buw.tmdt.plasma.ars.refinement.gr.service.converter.KGStoSRSDTOConverter;
...
@Service
public class GeneralizationRecommendationHandler {

    private static final Logger log = LoggerFactory.getLogger(GeneralizationRecommendationHandler.class);

    final UniversalKnowledgeApi universalKnowledgeApi;

    public GeneralizationRecommendationHandler(UniversalKnowledgeApi universalKnowledgeApi) {
        this.universalKnowledgeApi = universalKnowledgeApi;
    }

    public DataSourceSchemaDTO recommendGeneralization(String dataSourceUUID, DataSourceSchemaDTO combined) {
        SyntaxModelDTO syntaxModelDTO = combined.getSyntaxModelDTO();
        SemanticModelDTO semanticModelDTO = combined.getSemanticModelDTO();
